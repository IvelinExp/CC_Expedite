package org.cc.exp.security.ui;
...
import org.cc.exp.security.ui.security.AccessDeniedEvent;
...
public class MainLayout extends VerticalLayout implements ViewDisplay, ClickListener, ViewChangeListener {
	...
	public void postConstuct() {
		...
		final Label brand = new Label("CC Expedite");
	...
	private void logout() {
		AnonymousAuthenticationToken authenticationToken = new AnonymousAuthenticationToken(key, "anonymousUser",
	...
	public void onAccessDenied(org.vaadin.spring.events.Event<AccessDeniedEvent> event) {
		...
		if (event.getPayload().getCause() != null) {
			...
			if (event.getPayload().getViewToken().equals(ViewToken.USER)) {
				Notification.show("Access is denied.", "You must sign in before accessing User home.",
						Type.ERROR_MESSAGE);
			} else {
				Notification.show("Access is denied.", "You must sign in as Admin user before accessing Admin home.",
						Type.ERROR_MESSAGE);
			}
		}
	}
